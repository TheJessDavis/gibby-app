  ...
  fetch(APP+'/embed/instructors.json').then(function(r){return r.json()}).then(function(d){
    ...
    list.forEach(function(i){
      h+='<div class="gibby-artist" style="display:flex;flex-direction:column;align-items:center;text-align:center">'
        ...
        +((i.classes||[]).length?'<div style="margin-top:10px;font-size:.9em"><b>Upcoming classes</b>'+i.classes.map(function(c){return '<div><a href="'+esc(c.url)+'" target="_blank" rel="noopener">'+esc(c.title)+'</a> \u00b7 '+esc(c.when)+'</div>'}).join('')+'</div>':'')
